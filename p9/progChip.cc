...
void ProgChip::doLine(){
  step++;
  check(step < 300, "Error: progChip: doLine(): stepp is hire than 300 potencially an infinite loop");
  switch(tok){
...
void ProgChip::runExpression(){
  ...
  if(tok == Lex::NUMBER)
    ...
    check(var.inTable(lex.word()),"Error: ProgChip: runExpression():Identifier not found in table");
...
void ProgChip::doRead(){
  ...
  check(var.inTable(lex.word()),"Error: ProgChip: doRead(): variable not declared");
  int memLoc = var[lex.word()];
  getInput();
  store(memLoc);
...
void ProgChip:: doWrite(){
  ...
  runExpression();
  pop(-1);
  check(lex.atEnd(),"ERROR: ProgChip: doWrite(): invalid syntax, EOLN expected");
