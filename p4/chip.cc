...
Chip & Chip:: push(){
  check(size > memory, "Error: chip: Push: stack is full");
  size->set(accu);
  size--;
  return *this;
}
...
Chip & Chip:: store(int loc){
  check(loc<200 && loc >=0, "Error: chip: store: loc is not within the bound of the memory locations");
  memory[loc] = accu;
  return *this;
}

Chip & Chip:: load1(int loc){
  check(loc<200 && loc >=0, "Error: chip: load1: loc is not within the bound of the memory locations");
  register1 = memory[loc].get();
  return *this;
}

Chip & Chip:: load2(int loc){
  check(loc<200 && loc >=0, "Error: chip: load2: loc is not within the bound of the memory locations");
